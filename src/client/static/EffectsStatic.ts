...
export const PlayEffect = (name: EffectName, additional?: Map<string, any>) => {
    if (!Effects.get(name)) { warn('Effects Doesnt Exist!'); return }
    Effects.get(name)!(additional)
}

export const Effects = new Map<EffectName, (
    additional?: Map<string, any>
) => void>([])

Effects.set(EffectName.ReplicatePurchase, (additional) => {
    Events.PurchasePrompt.fire(additional?.get('productId') as number, player.UserId)
})

Effects.set(EffectName.ClickSound, () => {
